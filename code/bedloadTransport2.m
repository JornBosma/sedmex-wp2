...
%% CHECK
% crossFrac = TT_cross_frac;
% longFrac = TT_long_frac;
% 
% crossU = TT_U;
% longU = TT_U;
% 
% for i = 1:6
%     crossFrac.(i)(TT_cross_sign.(i)==1) = NaN;
%     crossU.(i) = TT_U.(i) .* crossFrac.(i);
% 
%     longFrac.(i)(TT_long_sign.(i)==1) = NaN;
%     longU.(i) = TT_U.(i) .* longFrac.(i);
% end
% 
% meanCrossFrac = mean(crossFrac,'omitmissing');
% meanLongFrac = mean(longFrac,'omitmissing');
% 
% figure('Position',[740, 957, 1719, 1336])
% plot(crossU.Time, crossU.L1C1VEC, 'b', 'LineWidth',2)
% hold on
% plot(longU.Time, longU.L1C1VEC, 'r', 'LineWidth',2)
...
% TT_phi_bMg_NaN = TT_phi_bMg(rowsWithoutNaNs, :);
% TT_phi_b10_NaN = TT_phi_b10(rowsWithoutNaNs, :);
% TT_phi_b50_NaN = TT_phi_b50(rowsWithoutNaNs, :);
% TT_phi_b90_NaN = TT_phi_b90(rowsWithoutNaNs, :);
% TT_phi_bFine_NaN = TT_phi_bFine(rowsWithoutNaNs, :);
% TT_phi_bCoarse_NaN = TT_phi_bCoarse(rowsWithoutNaNs, :);
% 
% TT_q_bMg_NaN = TT_q_bMg(rowsWithoutNaNs, :);
% TT_q_b10_NaN = TT_q_b10(rowsWithoutNaNs, :);
% TT_q_b50_NaN = TT_q_b50(rowsWithoutNaNs, :);
% TT_q_b90_NaN = TT_q_b90(rowsWithoutNaNs, :);
% TT_q_bFine_NaN = TT_q_bFine(rowsWithoutNaNs, :);
% TT_q_bCoarse_NaN = TT_q_bCoarse(rowsWithoutNaNs, :);
...
TT_qL_bMg_zeros = TT_q_bMg_zeros .* TT_long_frac .* TT_long_sign;
TT_qL_b10_zeros = TT_q_b10_zeros .* TT_long_frac .* TT_long_sign;
TT_qL_b50_zeros = TT_q_b50_zeros .* TT_long_frac .* TT_long_sign;
TT_qL_b90_zeros = TT_q_b90_zeros .* TT_long_frac .* TT_long_sign;
TT_qL_bFine_zeros = TT_q_bFine_zeros .* TT_long_frac .* TT_long_sign;
TT_qL_bCoarse_zeros = TT_q_bCoarse_zeros .* TT_long_frac .* TT_long_sign;

TT_qC_bMg_zeros = TT_q_bMg_zeros .* TT_cross_frac .* TT_cross_sign;
TT_qC_b10_zeros = TT_q_b10_zeros .* TT_cross_frac .* TT_cross_sign;
TT_qC_b50_zeros = TT_q_b50_zeros .* TT_cross_frac .* TT_cross_sign;
TT_qC_b90_zeros = TT_q_b90_zeros .* TT_cross_frac .* TT_cross_sign;
TT_qC_bFine_zeros = TT_q_bFine_zeros .* TT_cross_frac .* TT_cross_sign;
TT_qC_bCoarse_zeros = TT_q_bCoarse_zeros .* TT_cross_frac .* TT_cross_sign;

TT_qL_bMg_zeros{nanRowIndices, :} = 0;
TT_qL_b10_zeros{nanRowIndices, :} = 0;
TT_qL_b50_zeros{nanRowIndices, :} = 0;
TT_qL_b90_zeros{nanRowIndices, :} = 0;
TT_qL_bFine_zeros{nanRowIndices, :} = 0;
TT_qL_bCoarse_zeros{nanRowIndices, :} = 0;

TT_qC_bMg_zeros{nanRowIndices, :} = 0;
TT_qC_b10_zeros{nanRowIndices, :} = 0;
TT_qC_b50_zeros{nanRowIndices, :} = 0;
TT_qC_b90_zeros{nanRowIndices, :} = 0;
TT_qC_bFine_zeros{nanRowIndices, :} = 0;
TT_qC_bCoarse_zeros{nanRowIndices, :} = 0;

clearvars nanRows nanRowIndices


%% Check
A = TT_qL_bCoarse_zeros.L6C1VEC;
A(TT_long_sign.L6C1VEC>0) = 0;
B = TT_qC_bCoarse_zeros.L6C1VEC;
B(TT_cross_sign.L6C1VEC>0) = 0;

figure('Position',[740, 957, 1719, 1336])
yyaxis left
% plot(TT_q_bCoarse_zeros.Time, TT_q_bCoarse_zeros.L1C1VEC, 'k', 'LineWidth',2)
fill([TT_q_bCoarse_zeros.Time; flipud(TT_q_bCoarse_zeros.Time)], [TT_q_bCoarse_zeros.L6C1VEC; zeros(size(TT_q_bCoarse_zeros.L6C1VEC))], 'k', 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
% plot(TT_q_bCoarse_zeros.Time, A, 'r', 'LineWidth',2)
% plot(TT_q_bCoarse_zeros.Time, B, 'b', 'LineWidth',2)
fill([TT_q_bCoarse_zeros.Time; flipud(TT_q_bCoarse_zeros.Time)], [A; zeros(size(A))], 'r', 'FaceAlpha', 0.3, 'EdgeColor', 'none');
fill([TT_q_bCoarse_zeros.Time; flipud(TT_q_bCoarse_zeros.Time)], [B; zeros(size(B))], 'b', 'FaceAlpha', 0.3, 'EdgeColor', 'none');

yyaxis right
scatter(TT_q_bCoarse_zeros.Time, TT_cross_frac.L1C1VEC, '.b')
scatter(TT_q_bCoarse_zeros.Time, TT_long_frac.L1C1VEC, '.r')

time_seconds = seconds(TT_q_b10_zeros.Time - TT_q_b10_zeros.Time(1));

trapz(time_seconds, A)
trapz(time_seconds, B)
...
time_seconds = seconds(TT_q_b10_zeros.Time - TT_q_b10_zeros.Time(1));
time_seconds = time_seconds - time_seconds(1);
...
for i = 1:length(instruLocs)
    ...
    % timeStep = 600;
    % Mg(i) = sum(TT_q_bMg_zeros.(i)) * timeStep;
    % d10(i) = sum(TT_q_b10_zeros.(i)) * timeStep;
    % d50(i) = sum(TT_q_b50_zeros.(i)) * timeStep;
    % d90(i) = sum(TT_q_b90_zeros.(i)) * timeStep;
    % Fine(i) = sum(TT_q_bFine_zeros.(i)) * timeStep;
    % Coarse(i) = sum(TT_q_bCoarse_zeros.(i)) * timeStep;
...
% Convert datetime to duration and then to seconds
time_seconds = seconds(TT_qL_b10_zeros.Time - TT_qL_b10_zeros.Time(1));
time_seconds = time_seconds - time_seconds(1);

% Zero out positive values
negL_tt_bMg = TT_qL_bMg_zeros;
negL_tt_b10 = TT_qL_b10_zeros;
negL_tt_b50 = TT_qL_b50_zeros;
negL_tt_b90 = TT_qL_b90_zeros;
negL_tt_bFine = TT_qL_bFine_zeros;
negL_tt_bCoarse = TT_qL_bCoarse_zeros;

% Loop through each variable in the timetable
for varIdx = 1:width(negL_tt_bMg)
    % Get the current variable's data
    data_Mg = negL_tt_bMg{:, varIdx};
    data_10 = negL_tt_b10{:, varIdx};
    data_50 = negL_tt_b50{:, varIdx};
    data_90 = negL_tt_b90{:, varIdx};
    data_Fine = negL_tt_bFine{:, varIdx};
    data_Coarse = negL_tt_bCoarse{:, varIdx};
    ...
    negL_tt_bMg{:, varIdx} = data_Mg;
    negL_tt_b10{:, varIdx} = data_10;
    negL_tt_b50{:, varIdx} = data_50;
    negL_tt_b90{:, varIdx} = data_90;
    negL_tt_bFine{:, varIdx} = data_Fine;
    negL_tt_bCoarse{:, varIdx} = data_Coarse;
end

% Zero out negative values
posL_tt_bMg = TT_qL_bMg_zeros;
posL_tt_b10 = TT_qL_b10_zeros;
posL_tt_b50 = TT_qL_b50_zeros;
posL_tt_b90 = TT_qL_b90_zeros;
posL_tt_bFine = TT_qL_bFine_zeros;
posL_tt_bCoarse = TT_qL_bCoarse_zeros;

% Loop through each variable in the timetable
for varIdx = 1:width(posL_tt_bMg)
    % Get the current variable's data
    data_Mg = posL_tt_bMg{:, varIdx};
    data_10 = posL_tt_b10{:, varIdx};
    data_50 = posL_tt_b50{:, varIdx};
    data_90 = posL_tt_b90{:, varIdx};
    data_Fine = posL_tt_bFine{:, varIdx};
    data_Coarse = posL_tt_bCoarse{:, varIdx};
    ...
    % Assign the modified data to the new timetable
    posL_tt_bMg{:, varIdx} = data_Mg;
    posL_tt_b10{:, varIdx} = data_10;
    posL_tt_b50{:, varIdx} = data_50;
    posL_tt_b90{:, varIdx} = data_90;
    posL_tt_bFine{:, varIdx} = data_Fine;
    posL_tt_bCoarse{:, varIdx} = data_Coarse;
end

for i = 1:length(instruLocs)
    
    % Integrate using the trapezoidal rule
    Mg(i) = trapz(time_seconds, TT_qL_bMg_zeros.(i));
    d10(i) = trapz(time_seconds, TT_qL_b10_zeros.(i));
    d50(i) = trapz(time_seconds, TT_qL_b50_zeros.(i));
    d90(i) = trapz(time_seconds, TT_qL_b90_zeros.(i));
    Fine(i) = trapz(time_seconds, TT_qL_bFine_zeros.(i));
    Coarse(i) = trapz(time_seconds, TT_qL_bCoarse_zeros.(i));

    % Integrate by summation
    % Mg(i) = sum(TT_qL_bMg_zeros.(i)) * timeStep;
    % d10(i) = sum(TT_qL_b10_zeros.(i)) * timeStep;
    % d50(i) = sum(TT_qL_b50_zeros.(i)) * timeStep;
    % d90(i) = sum(TT_qL_b90_zeros.(i)) * timeStep;
    % Fine(i) = sum(TT_qL_bFine_zeros.(i)) * timeStep;
    % Coarse(i) = sum(TT_qL_bCoarse_zeros.(i)) * timeStep;

    % Display the result
    % fprintf('Total amount of bedload transport: %.2f m^3/m width\n', totalBedload);

    % Perform the integration over negative values
    Mg_neg(i) = trapz(time_seconds, negL_tt_bMg.(i));
    d10_neg(i) = trapz(time_seconds, negL_tt_b10.(i));
    d50_neg(i) = trapz(time_seconds, negL_tt_b50.(i));
    d90_neg(i) = trapz(time_seconds, negL_tt_b90.(i));
    Fine_neg(i) = trapz(time_seconds, negL_tt_bFine.(i));
    Coarse_neg(i) = trapz(time_seconds, negL_tt_bCoarse.(i));

    % Integrate by summation
    % Mg_neg(i) = sum(negL_tt_bMg.(i)) * timeStep;
    % d10_neg(i) = sum(negL_tt_b10.(i)) * timeStep;
    % d50_neg(i) = sum(negL_tt_b50.(i)) * timeStep;
    % d90_neg(i) = sum(negL_tt_b90.(i)) * timeStep;
    % Fine_neg(i) = sum(negL_tt_bFine.(i)) * timeStep;
    % Coarse_neg(i) = sum(negL_tt_bCoarse.(i)) * timeStep;

    % Perform the integration over negative values
    Mg_pos(i) = trapz(time_seconds, posL_tt_bMg.(i));
    d10_pos(i) = trapz(time_seconds, posL_tt_b10.(i));
    d50_pos(i) = trapz(time_seconds, posL_tt_b50.(i));
    d90_pos(i) = trapz(time_seconds, posL_tt_b90.(i));
    Fine_pos(i) = trapz(time_seconds, posL_tt_bFine.(i));
    Coarse_pos(i) = trapz(time_seconds, posL_tt_bCoarse.(i));

    % Integrate by summation
    % Mg_pos(i) = sum(posL_tt_bMg.(i)) * timeStep;
    % d10_pos(i) = sum(posL_tt_b10.(i)) * timeStep;
    % d50_pos(i) = sum(posL_tt_b50.(i)) * timeStep;
    % d90_pos(i) = sum(posL_tt_b90.(i)) * timeStep;
    % Fine_pos(i) = sum(posL_tt_bFine.(i)) * timeStep;
    % Coarse_pos(i) = sum(posL_tt_bCoarse.(i)) * timeStep;
end
...
% Add F/C and d10/d90 ratios to the table
FC_ratio = Fine./Coarse;
FC_ratio = [FC_ratio; mean(FC_ratio)];
totalBedload_netL.FC = FC_ratio;
d10d90_ratio = d10./d90;
...
% Convert datetime to duration and then to seconds
time_seconds = seconds(TT_qC_b10_zeros.Time - TT_qC_b10_zeros.Time(1));
time_seconds = time_seconds - time_seconds(1);

% Zero out positive values
negC_tt_bMg = TT_qC_bMg_zeros;
negC_tt_b10 = TT_qC_b10_zeros;
negC_tt_b50 = TT_qC_b50_zeros;
negC_tt_b90 = TT_qC_b90_zeros;
negC_tt_bFine = TT_qC_bFine_zeros;
negC_tt_bCoarse = TT_qC_bCoarse_zeros;

% Loop through each variable in the timetable
for varIdx = 1:width(negC_tt_bMg)
    % Get the current variable's data
    data_Mg = negC_tt_bMg{:, varIdx};
    data_10 = negC_tt_b10{:, varIdx};
    data_50 = negC_tt_b50{:, varIdx};
    data_90 = negC_tt_b90{:, varIdx};
    data_Fine = negC_tt_bFine{:, varIdx};
    data_Coarse = negC_tt_bCoarse{:, varIdx};
    ...
    % Assign the modified data to the new timetable
    negC_tt_bMg{:, varIdx} = data_Mg;
    negC_tt_b10{:, varIdx} = data_10;
    negC_tt_b50{:, varIdx} = data_50;
    negC_tt_b90{:, varIdx} = data_90;
    negC_tt_bFine{:, varIdx} = data_Fine;
    negC_tt_bCoarse{:, varIdx} = data_Coarse;
end

% Zero out negative values
posC_tt_bMg = TT_qC_bMg_zeros;
posC_tt_b10 = TT_qC_b10_zeros;
posC_tt_b50 = TT_qC_b50_zeros;
posC_tt_b90 = TT_qC_b90_zeros;
posC_tt_bFine = TT_qC_bFine_zeros;
posC_tt_bCoarse = TT_qC_bCoarse_zeros;

% Loop through each variable in the timetable
for varIdx = 1:width(posC_tt_bMg)
    % Get the current variable's data
    data_Mg = posC_tt_bMg{:, varIdx};
    data_10 = posC_tt_b10{:, varIdx};
    data_50 = posC_tt_b50{:, varIdx};
    data_90 = posC_tt_b90{:, varIdx};
    data_Fine = posC_tt_bFine{:, varIdx};
    data_Coarse = posC_tt_bCoarse{:, varIdx};
    ...
    % Assign the modified data to the new timetable
    posC_tt_bMg{:, varIdx} = data_Mg;
    posC_tt_b10{:, varIdx} = data_10;
    posC_tt_b50{:, varIdx} = data_50;
    posC_tt_b90{:, varIdx} = data_90;
    posC_tt_bFine{:, varIdx} = data_Fine;
    posC_tt_bCoarse{:, varIdx} = data_Coarse;
end

for i = 1:length(instruLocs)
    
    % Integrate using the trapezoidal rule
    Mg(i) = trapz(time_seconds, TT_qC_bMg_zeros.(i));
    d10(i) = trapz(time_seconds, TT_qC_b10_zeros.(i));
    d50(i) = trapz(time_seconds, TT_qC_b50_zeros.(i));
    d90(i) = trapz(time_seconds, TT_qC_b90_zeros.(i));
    Fine(i) = trapz(time_seconds, TT_qC_bFine_zeros.(i));
    Coarse(i) = trapz(time_seconds, TT_qC_bCoarse_zeros.(i));

    % Integrate by summation
    % Mg(i) = sum(TT_qC_bMg_zeros.(i)) * timeStep;
    % d10(i) = sum(TT_qC_b10_zeros.(i)) * timeStep;
    % d50(i) = sum(TT_qC_b50_zeros.(i)) * timeStep;
    % d90(i) = sum(TT_qC_b90_zeros.(i)) * timeStep;
    % Fine(i) = sum(TT_qC_bFine_zeros.(i)) * timeStep;
    % Coarse(i) = sum(TT_qC_bCoarse_zeros.(i)) * timeStep;

    % Display the result
    % fprintf('Total amount of bedload transport: %.2f m^3/m width\n', totalBedload);

    % Perform the integration over negative values
    Mg_neg(i) = trapz(time_seconds, negC_tt_bMg.(i));
    d10_neg(i) = trapz(time_seconds, negC_tt_b10.(i));
    d50_neg(i) = trapz(time_seconds, negC_tt_b50.(i));
    d90_neg(i) = trapz(time_seconds, negC_tt_b90.(i));
    Fine_neg(i) = trapz(time_seconds, negC_tt_bFine.(i));
    Coarse_neg(i) = trapz(time_seconds, negC_tt_bCoarse.(i));

    % Integrate by summation
    % Mg_neg(i) = sum(negC_tt_bMg.(i)) * timeStep;
    % d10_neg(i) = sum(negC_tt_b10.(i)) * timeStep;
    % d50_neg(i) = sum(negC_tt_b50.(i)) * timeStep;
    % d90_neg(i) = sum(negC_tt_b90.(i)) * timeStep;
    % Fine_neg(i) = sum(negC_tt_bFine.(i)) * timeStep;
    % Coarse_neg(i) = sum(negC_tt_bCoarse.(i)) * timeStep;

    % Perform the integration over negative values
    Mg_pos(i) = trapz(time_seconds, posC_tt_bMg.(i));
    d10_pos(i) = trapz(time_seconds, posC_tt_b10.(i));
    d50_pos(i) = trapz(time_seconds, posC_tt_b50.(i));
    d90_pos(i) = trapz(time_seconds, posC_tt_b90.(i));
    Fine_pos(i) = trapz(time_seconds, posC_tt_bFine.(i));
    Coarse_pos(i) = trapz(time_seconds, posC_tt_bCoarse.(i));

    % Integrate by summation
    % Mg_pos(i) = sum(posC_tt_bMg.(i)) * timeStep;
    % d10_pos(i) = sum(posC_tt_b10.(i)) * timeStep;
    % d50_pos(i) = sum(posC_tt_b50.(i)) * timeStep;
    % d90_pos(i) = sum(posC_tt_b90.(i)) * timeStep;
    % Fine_pos(i) = sum(posC_tt_bFine.(i)) * timeStep;
    % Coarse_pos(i) = sum(posC_tt_bCoarse.(i)) * timeStep;
end
...
%% Visualisation: Bed Shear Stress / Gross & Net BL Transport Rate (pdf: Fine)
...
for i = 1:length(instruLocs)
    ...
    % Create the KDE plot
    [f, xi] = ksdensity(data, 'BoundaryCorrection','reflection',...
        'Support','nonnegative', 'NumPoints',numPoints);
    plot(xi, f, 'LineWidth',4, 'DisplayName',instruLocs{i}, 'Color',newcolors(i,:))
    % xline(mean(data, 'omitnan'), 'LineWidth',2, 'Color',newcolors(i,:))
...
for i = 1:length(instruLocs)
    ...
    % Extract the variable data for the current location
    data = TT_q_bFine_zeros.(i);

    % Remove NaN values
    % data = data(~isnan(data));

    % Remove zero-rows
    rowsToRemove = all(data == 0, 2);
    data(rowsToRemove, :) = [];
...
% Add title, labels and legend
xlabel('q_{b,Fine,gross} (m^2 s^{-1})')
...
for i = 1:length(instruLocs)
    ...
    % % Apply Gaussian smoothing
    % gaussianWindowSize = 1000; % Adjust window size for more or less smoothing
    % smoothed_f = smoothdata(f, 'gaussian', gaussianWindowSize);
    % 
...
%% Visualisation: Bed Shear Stress / Gross & Net BL Transport Rate (pdf: Coarse)

% Number of points for higher resolution
numPoints = 2e3; % Increase this value for higher resolution

f6e = figure('Position',[740, 1665, 1719, 628]);
tiledlayout(1,3, "TileSpacing","tight")
...
%% Visualisation: Bed Shear Stress / Gross & Net BL Transport Rate (pdf: Fine & Coarse)
...
%% Reorganisation
varNames = {'onshore', 'offshore', 'easterly', 'westerly', 'gross'};

SUM_Mg = totalBedload_C_pos.Mg_pos(1:end-1) + abs(totalBedload_C_neg.Mg_neg(1:end-1)) + totalBedload_L_pos.Mg_pos(1:end-1) + abs(totalBedload_L_neg.Mg_neg(1:end-1));
SUM_d10 = totalBedload_C_pos.d10_pos(1:end-1) + abs(totalBedload_C_neg.d10_neg(1:end-1)) + totalBedload_L_pos.d10_pos(1:end-1) + abs(totalBedload_L_neg.d10_neg(1:end-1));
SUM_d50 = totalBedload_C_pos.d50_pos(1:end-1) + abs(totalBedload_C_neg.d50_neg(1:end-1)) + totalBedload_L_pos.d50_pos(1:end-1) + abs(totalBedload_L_neg.d50_neg(1:end-1));
SUM_d90 = totalBedload_C_pos.d90_pos(1:end-1) + abs(totalBedload_C_neg.d90_neg(1:end-1)) + totalBedload_L_pos.d90_pos(1:end-1) + abs(totalBedload_L_neg.d90_neg(1:end-1));
SUM_Fine = totalBedload_C_pos.Fine_pos(1:end-1) + abs(totalBedload_C_neg.Fine_neg(1:end-1)) + totalBedload_L_pos.Fine_pos(1:end-1) + abs(totalBedload_L_neg.Fine_neg(1:end-1));
SUM_Coarse = totalBedload_C_pos.Coarse_pos(1:end-1) + abs(totalBedload_C_neg.Coarse_neg(1:end-1)) + totalBedload_L_pos.Coarse_pos(1:end-1) + abs(totalBedload_L_neg.Coarse_neg(1:end-1));

% BLV_Mg = table(totalBedload_C_pos.Mg_pos(1:end-1), totalBedload_C_neg.Mg_neg(1:end-1), totalBedload_L_pos.Mg_pos(1:end-1), totalBedload_L_neg.Mg_neg(1:end-1), SUM_Mg, totalBedload_gross.Mg(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_Mg = table(totalBedload_C_pos.Mg_pos(1:end-1), totalBedload_C_neg.Mg_neg(1:end-1), totalBedload_L_pos.Mg_pos(1:end-1), totalBedload_L_neg.Mg_neg(1:end-1), totalBedload_gross.Mg(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_Mg = flipud(abs(BLV_Mg));

% BLV_d10 = table(totalBedload_C_pos.d10_pos(1:end-1), totalBedload_C_neg.d10_neg(1:end-1), totalBedload_L_pos.d10_pos(1:end-1), totalBedload_L_neg.d10_neg(1:end-1), SUM_d10, totalBedload_gross.d10(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_d10 = table(totalBedload_C_pos.d10_pos(1:end-1), totalBedload_C_neg.d10_neg(1:end-1), totalBedload_L_pos.d10_pos(1:end-1), totalBedload_L_neg.d10_neg(1:end-1), totalBedload_gross.d10(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_d10 = flipud(abs(BLV_d10));

% BLV_d50 = table(totalBedload_C_pos.d50_pos(1:end-1), totalBedload_C_neg.d50_neg(1:end-1), totalBedload_L_pos.d50_pos(1:end-1), totalBedload_L_neg.d50_neg(1:end-1), SUM_d50, totalBedload_gross.d50(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_d50 = table(totalBedload_C_pos.d50_pos(1:end-1), totalBedload_C_neg.d50_neg(1:end-1), totalBedload_L_pos.d50_pos(1:end-1), totalBedload_L_neg.d50_neg(1:end-1), totalBedload_gross.d50(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_d50 = flipud(abs(BLV_d50));

% BLV_d90 = table(totalBedload_C_pos.d90_pos(1:end-1), totalBedload_C_neg.d90_neg(1:end-1), totalBedload_L_pos.d90_pos(1:end-1), totalBedload_L_neg.d90_neg(1:end-1), SUM_d90, totalBedload_gross.d90(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_d90 = table(totalBedload_C_pos.d90_pos(1:end-1), totalBedload_C_neg.d90_neg(1:end-1), totalBedload_L_pos.d90_pos(1:end-1), totalBedload_L_neg.d90_neg(1:end-1), totalBedload_gross.d90(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_d90 = flipud(abs(BLV_d90));

% BLV_Fine = table(totalBedload_C_pos.Fine_pos(1:end-1), totalBedload_C_neg.Fine_neg(1:end-1), totalBedload_L_pos.Fine_pos(1:end-1), totalBedload_L_neg.Fine_neg(1:end-1), SUM_Fine, totalBedload_gross.Fine(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_Fine = table(totalBedload_C_pos.Fine_pos(1:end-1), totalBedload_C_neg.Fine_neg(1:end-1), totalBedload_L_pos.Fine_pos(1:end-1), totalBedload_L_neg.Fine_neg(1:end-1), totalBedload_gross.Fine(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_Fine = flipud(abs(BLV_Fine));

% BLV_Coarse = table(totalBedload_C_pos.Coarse_pos(1:end-1), totalBedload_C_neg.Coarse_neg(1:end-1), totalBedload_L_pos.Coarse_pos(1:end-1), totalBedload_L_neg.Coarse_neg(1:end-1), SUM_Coarse, totalBedload_gross.Coarse(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
BLV_Coarse = table(totalBedload_C_pos.Coarse_pos(1:end-1), totalBedload_C_neg.Coarse_neg(1:end-1), totalBedload_L_pos.Coarse_pos(1:end-1), totalBedload_L_neg.Coarse_neg(1:end-1), totalBedload_gross.Coarse(1:end-1), 'VariableNames',varNames, 'RowNames',instruLocs);
...
colororder([brewermap(4,'Set3'); [0,0,0]])

% Add figure annotations
% annotation('textbox', [0.92, 0.84, 0.1, 0.1], 'String','(a)', 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
annotation('textbox', [0.92, 0.78, 0.1, 0.1], 'String','M_G', 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
annotation('textbox', [0.92, 0.635, 0.1, 0.1], 'String','D_{10}', 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
annotation('textbox', [0.92, 0.49, 0.1, 0.1], 'String','D_{50}', 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
annotation('textbox', [0.92, 0.34, 0.1, 0.1], 'String','D_{90}', 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
annotation('textbox', [0.92, 0.21, 0.1, 0.1], 'String',{'0.25';'mm'}, 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
annotation('textbox', [0.92, 0.06, 0.1, 0.1], 'String',{'2.00';'mm'}, 'EdgeColor','none', 'FontSize',fontsize, 'FontWeight','bold');
...
for i = 1:6
    ...
    % text(.6, 3.4e3, GS_headers{i}, 'FontSize',fontsize, 'HorizontalAlignment','left')
